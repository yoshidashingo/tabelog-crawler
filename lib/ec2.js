...
export async function createEC2AndRunConsumer(EC2_COUNT) {
    ...
    const instanceIDArr = data.Instances.map(i => i.InstanceId);
    console.log("instanceIDArr:", instanceIDArr);
    ...
    for (let i = 0; i < instanceIDArr.length; i++) {
        await checkEC2(instanceIDArr[i]);
    }
